    ...
    foreach ( $data as $row )
    {
        ...
        if ( key_exists( $fieldname, $_POST) )
        {
            ...
            if ( in_array( $type, explode("," , "INTEGER,INT,SMALLINT,TINYINT,MEDIUMINT,BIGINT" ) ) )
            {
                ...
                {
                    $msg = $sent_value . "please enter the number";
            ...
            if ( in_array( $type, explode("," , "VARCHAR,TEXT" ) ) )
            {
                ...
                {
                    $msg = "Please enter max $length characters";
                    $_SESSION['errors'][ "$fieldname" . "_error" ] = $msg;
                }
            }
            //DATE type
...
function ValidateUsrPassword( $password )
{
    ...
    {
        $_SESSION['errors']['usr_password_error'] = "min 8 characters";
        return false;
    }
...
/*function RequiredVoornaam(){
    if (strlen($_POST["usr_last_name"]) < 1){
        $msg = "This field is required";
        $_SESSION['errors'][ "usr_last_name_error" ] = $msg;
    }
}
function RequiredNaam(){
    if (strlen($_POST["usr_name"]) < 1){
        $msg = "This field is required";
        $_SESSION['errors'][ "usr_name_error" ] = $msg;
    }
}*/
...
// Validate e-mail
    ...
    {
        $_SESSION['errors']['usr_email_error'] = "Wrong email address!";
        return false;
    }
}
...
function CheckUniqueUsrEmail( $email )
{
    ...
    if (count($rows) > 0)
    {
        $_SESSION['errors']['usr_email_error'] = "This email address is already registered";
        return false;
    }
